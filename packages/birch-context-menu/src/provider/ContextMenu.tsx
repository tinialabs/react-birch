...
export const ContextMenuProvider = ({ children }) => {
    ...
    const showContextMenu = (data: IContextMenuData | Promise<IContextMenuData>, newpos?: IPosition) => {

        hideContextMenu();

        let handleActive = true;

        if (lastCapturedCtxMenuEvent.current && !lastCapturedCtxMenuEvent.current.defaultPrevented) {
        ...
        Promise.resolve(data)
            .then((ctxMenuData) => {
                ...
                pos.current = ((newpos && newpos.x > -1 && newpos.y > -1) ?
                    newpos
                    : lastCapturedCtxMenuEvent.current ?
                        { x: lastCapturedCtxMenuEvent.current.clientX, y: lastCapturedCtxMenuEvent.current.clientY }
                        : { x: 0, y: 0 })
    ...
    return <MenuReactContext.Provider value={{
        showContextMenu,
        hideContextMenu,
        pos: pos.current,
        visible,
        ...
        data.forEach((menuGroup, _i) => {
            ...
            const items = menuGroup.map((item) => {
                ...
                if ((item as ITextMenuItem).onClick) {
                    ...
                    return (
                        <StyledMenuItem key={item.label} className={`menu-item ${item.disabled ? 'disabled' : ''}`}>
                ...
                if ((item as ISubMenuItem).submenu) {
                    return (
                        <StyledMenuItem key={item.label} className={`menu-item submenu-item ${item.disabled ? 'disabled' : ''}`}>
