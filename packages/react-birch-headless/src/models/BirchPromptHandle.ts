import {
  ...
  IDisposable,
  EventEmitter
} from 'react-birch-event-emitter'
import {
  IBirchPromptHandle,
  IBirchTreeViewItemPromptProps
} from 'react-birch-types'
import { bindInputElement } from '../components/BirchTreeViewItemPrompt'
 ...
 */
export abstract class BirchPromptHandle implements IBirchPromptHandle {
  public readonly $: HTMLInputElement

  public readonly BirchTreeViewItemPrompt: (
  ...
  constructor() {
    ...
    this.BirchTreeViewItemPrompt = bindInputElement(this.$)
    ...
    if (!isProduction) {
      void delay(2000).then(() => {
        if (!this._destroyed && !this.$.isConnected) {
          console.error(
            `BirchPromptHandle created but never used. A potential memory leak vector.`
          )
        }
      })
    }
  ...
  public destroy(): void {
    ...
    if (!isProduction) {
      void delay(1000).then(() => {
        if (this.$.isConnected) {
          console.error(
            `[INTERNAL] A BirchPromptHandle associated HTMLInputElement, which is now obsolete, is still mounted in the DOM. PromptHandles are useless once destroyed.`
          )
        }
      })
    }
