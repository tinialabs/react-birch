...
function spliceTypedArray(
  ...
  const a = new Uint32Array(
    arr.length - deleteCount + (items ? items.length : 0)
  )
...
export class BirchFolder extends BirchItem implements IBirchItem, IBirchFolder {
  ...
  protected _children: Array<IBirchItem | IBirchFolder>
  ...
  public insertItem(item: IBirchItem | IBirchFolder) {
    ...
    this._children.sort(
      this.root.host.sortComparator || BirchFolder.defaultSortComparator
    )
    ...
    if (leadingSibling) {
      ...
      relativeInsertionIndex =
        siblingIdx +
        (leadingSibling instanceof BirchFolder && leadingSibling.expanded
          ? leadingSibling._branchSize
          : 0)
    } else {
    ...
    master.setFlattenedBranch(
      spliceTypedArray(master.flattenedBranch, absInsertionIndex, 0, branch)
    )
  }
  ...
  public unlinkItem(
    ...
    if (item instanceof BirchFolder && item.expanded) {
      item.setFlattenedBranch(
        master.flattenedBranch.slice(
          removalBeginIdx + 1,
          removalBeginIdx + branchSizeDecrease
        )
      )
    }
    master.setFlattenedBranch(
  ...
  public mv(to: BirchFolder, newName: string = this.label) {
    if (to.path === this.parent.path && newName === this.label) {
      // return
    }
    ...
    if (typeof this.watchTerminator === 'function') {
      ...
      if (this._children) {
        this.watchTerminator = this._superv.supervisedWatch(
          this.path,
  ...
  protected expandBranch(branch: BirchFolder) {
    ...
    if (this !== branch && this.flattenedBranch) {
      ...
      this.setFlattenedBranch(
        spliceTypedArray(
          this.flattenedBranch,
          injectionStartIdx,
          0,
          branch.flattenedBranch
        )
      )
  ...
  protected shrinkBranch(branch: BirchFolder) {
    ...
    if (this !== branch && this.flattenedBranch) {
      ...
      branch.setFlattenedBranch(
        this.flattenedBranch.slice(
          removalStartIdx,
          removalStartIdx + branch._branchSize
        )
      )
      this.setFlattenedBranch(
        spliceTypedArray(
  ...
  protected async hardReloadChildren() {
    ...
    const rawItemsPre =
      (await this.root.host.getChildren({
        path: this.path,
    ...
    for (let i = 0; i < rawItems.length; i++) {
      ...
      const { type, label } = rawTreeItem
      const child = new (
        type === EnumTreeItemType.Folder ? BirchFolder : BirchItem
      )(this.root, this._superv, this, label || 'untitled', rawTreeItem)
  ...
  private handleWatchEvent = async (event: IBirchWatcherEvent) => {
    ...
    if (event.type === EnumBirchWatchEvent.DidChangeTreeData) {
      ...
      if (target) {
        ...
        const newPath = target.parent
          ? this.root.pathfx.join(target.parent.path, rawItem.label)
          : rawItem.label
        ...
        if (oldPath !== newPath) {
          ...
          if (target.type === EnumTreeItemType.Folder) {
            await (target as BirchFolder).didChangeItem()
          }
        }
        ...
        console.log(
          'BirchFolder handleWatchEvent could not find item id',
          rawItem.tid
        )
      }
      ...
      if (this.root.pathfx.isRelative(oldPath)) {
        throw new TypeError(`oldPath must be absolute`)
      }
      if (this.root.pathfx.isRelative(newPath)) {
      ...
      BirchItem.checkRawItem(rawTreeItem)
      const newItem = new (
        rawTreeItem.type === EnumTreeItemType.Folder ? BirchFolder : BirchItem
      )(this.root, this._superv, this, rawTreeItem.label, rawTreeItem)
    ...
    } /* Maybe generic change event */ else {
      for (let i = 0; i < this._children.length; i++) {
