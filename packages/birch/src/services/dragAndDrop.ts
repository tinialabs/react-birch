...
export class DragAndDropService {
    ...
    public onDragAndDrop(cb: (target: BirchItem | BirchFolder, to: BirchFolder) => void) {
    ...
    public handleDragStart = (ev: React.DragEvent, item: BirchItem | BirchFolder) => {
        this.beingDraggedItem = item
        const isDirAndExpanded = (item.type === EnumTreeItemType.Folder && (item as BirchFolder).expanded)
        this.wasBeingDraggedExpanded = isDirAndExpanded
        if (isDirAndExpanded) {
            (item as BirchFolder).setCollapsed()
        }
        this.beingDraggedDecoration.addTarget(item as any, DecorationTargetMatchMode.Self)
    }

    public handleDragEnd = (ev: React.DragEvent, item: BirchItem | BirchFolder) => {
    ...
    public handleDragEnter = (ev: React.DragEvent, item: BirchItem | BirchFolder) => {
        ...
        const newPotentialParent: BirchFolder = (item.type === EnumTreeItemType.Folder && (item as BirchFolder).expanded)
            ? item as BirchFolder
            : item.parent
        ...
        if (this.potentialParent !== newPotentialParent && newPotentialParent !== this.beingDraggedItem.parent) {
            ...
            this.draggedOverDecoration.addTarget(this.potentialParent, DecorationTargetMatchMode.SelfAndChildren)
        }

        if (this.potentialParent !== item && item.type === EnumTreeItemType.Folder) {
    ...
    public handleDrop = (ev: React.DragEvent) => {
        ...
        this.events.emit(DnDServiceEvent.FinishDnD, this.beingDraggedItem, this.potentialParent)
