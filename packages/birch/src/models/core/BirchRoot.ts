import { Disposable, DisposablesComposite, IDisposable, EventEmitter } from 'birch-event-emitter'
...
import { BirchItemOrFolder } from '../../models'

import { 
	EnumTreeItemType, ITreeItem, 
} from '../../types'
...
export class BirchRoot extends BirchFolder {
	...
	private onceItemVisibleWatchers: WeakMap<BirchItemOrFolder, { item: BirchItemOrFolder, disposer: DisposablesComposite, callbacks: Set<(item: BirchItemOrFolder) => void> }>
	private onceFolderExpandedWatchers: WeakMap<BirchFolder, Set<(folder: BirchFolder, nowExpanded: boolean, visibleAtSurface: boolean) => void>>
	private onceDisposedWatchers: WeakMap<BirchItemOrFolder, Set<(target: BirchItemOrFolder) => void>>
	private onceParentChangedWatchers: WeakMap<BirchItemOrFolder, Set<(target: BirchItemOrFolder, prevParent: BirchFolder, newParent: BirchFolder) => void>>
	...
	constructor(host: IBirchCoreHost<any>, root: string, viewId: string) {
		...
		if (pathfx.isRelative(root)) {
			throw new Error(`BirchRoot path must be absolute. Example: 'C:\\Users\\Desktop' or '/home/desktop'`)
		}
		...
		const superv: IBirchTreeSupervisor = {
			...
			notifyWillProcessWatchEvent: (t: BirchFolder, event: IBirchWatcherEvent) => {
				this.events.emit(BirchTreeEvent.WillProcessWatchEvent, t, event)
			},
			notifyDidProcessWatchEvent: (t: BirchFolder, event: IBirchWatcherEvent) => {
			...
			notifyWillChangeParent: (t: BirchItemOrFolder, prevParent: BirchFolder, newParent: BirchFolder) => {
				this.events.emit(BirchTreeEvent.WillChangeParent, t, prevParent, newParent)
			},
			notifyDidChangeParent: (t: BirchItemOrFolder, prevParent: BirchFolder, newParent: BirchFolder) => {
				...
				this.events.emit(BirchTreeEvent.DidChangeParent, t, prevParent, newParent)
			},
			...
			notifyWillChangeExpansionState: (t: BirchFolder, nowExpanded: boolean) => {
				this.events.emit(BirchTreeEvent.WillChangeExpansionState, t, nowExpanded)
			},
			notifyDidChangeExpansionState: (t: BirchFolder, nowExpanded: boolean) => {
				...
				this.events.emit(BirchTreeEvent.DidChangeExpansionState, t, nowExpanded, isVisibleAtSurface)
			},
			supervisedWatch: (path: string, callback: BirchWatcherCallback): BirchWatchTerminator => {
		...
		host.onDidChangeTreeData(async (preItemRaw) => {
			...
			const item = preItemRaw ? await host.getTreeItem(preItemRaw) : this as ITreeItem
	...
	public onDidChangeTreeData(cb: (item: BirchItemOrFolder) => void): IDisposable {
		return this.events.on(BirchTreeEvent.DidChangeTreeData, cb)
	}

	public onDidChangeFolderExpansionState(cb: (folder: BirchFolder, nowExpanded: boolean, visibleAtSurface: boolean) => void): IDisposable {
		return this.events.on(BirchTreeEvent.DidChangeExpansionState, cb)
	}

	public onWillChangeFolderExpansionState(cb: (folder: BirchFolder, nowExpanded: boolean) => void): IDisposable {
		return this.events.on(BirchTreeEvent.WillChangeExpansionState, cb)
	}

	public onWillProcessWatchEvent(cb: (folder: BirchFolder, event: IBirchWatcherEvent) => void): IDisposable {
		return this.events.on(BirchTreeEvent.WillProcessWatchEvent, cb)
	}
	public onDidProcessWatchEvent(cb: (folder: BirchFolder, event: IBirchWatcherEvent) => void): IDisposable {
	...
	public onDidChangeParent(callback: (target: BirchItemOrFolder, prevParent: BirchFolder, newParent: BirchFolder) => void): IDisposable {
		return this.events.on(BirchTreeEvent.DidChangeParent, callback)
	}

	public onWillChangeParent(callback: (target: BirchItemOrFolder, prevParent: BirchFolder, newParent: BirchFolder) => void): IDisposable {
		return this.events.on(BirchTreeEvent.WillChangeParent, callback)
	}

	public onDidChangePath(callback: (target: BirchItemOrFolder) => void): IDisposable {
		return this.events.on(BirchTreeEvent.DidChangePath, callback)
	}

	public onOnceDisposed(target: BirchItemOrFolder, callback: (target: BirchItemOrFolder) => void): IDisposable {
		if (target.disposed) {
			callback(target)
	 ...
	 */
	public async forceLoadItemEntryAtPath(path: string): Promise<BirchFolder | BirchItem> {
		const pathfrags = this.pathfx.isRelative(path) ? this.pathfx.splitPath(path) : this.walkPathTillRelative(path)
		if (pathfrags.length === 0) {
			return this
		}
	...
	public async flushEventQueue() {
		...
		const promise = pSeries(this.changeEventDispatchQueue.map((path) => async () => {
			...
			if (watcher && typeof watcher.callback === 'function') {
				await watcher.callback({ type: EnumBirchWatchEvent.Changed, tid: path[1], folder: path[0] })
			}
			return null
		}) as any)
		// reset the queue
