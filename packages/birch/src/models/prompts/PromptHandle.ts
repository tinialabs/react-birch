import { DisposablesComposite, IDisposable, EventEmitter } from 'birch-event-emitter'
import { bindInputElement, BirchTreeViewItemPromptProps } from '../../components/BirchTreeViewItemPrompt'
...
export abstract class PromptHandle {
	...
	public readonly ProxiedInput: (props: BirchTreeViewItemPromptProps) => JSX.Element
	...
	public onChange(callback: (value: string) => void): IDisposable {
		return this.disposables.add(
			this.events.on(PromptEvent.Change, callback))
	}
	...
	private handleKeydown = async (ev) => {
		if (ev.key === 'Escape') {
			if ((await Promise.all(
				this.events.emitWithReturn<boolean>(PromptEvent.Cancel, this.$.value))
			).some((r) => r === false)) {
				return
			}
			this.destroy()
		...
		if (ev.key === 'Enter') {
			...
			this.$.disabled = true
			if ((await Promise.all(
				this.events.emitWithReturn<boolean>(PromptEvent.Commit, this.$.value))
			).some((r) => r === false)) {
