import { BirchFolder, BirchItem, BirchRoot } from '../../models'
...
export class DecorationsManager implements IDisposable {
	...
	private decorationsMeta: WeakMap<BirchItem | BirchFolder, IDecorationMeta> = new WeakMap()
	...
	public getDecorations(item: BirchItem | BirchFolder): ClasslistComposite {
		...
		if (!item || (item.type !== EnumTreeItemType.Item && item.type !== EnumTreeItemType.Folder)) {
			return null
		}
	...
	public getDecorationData(item: BirchItem | BirchFolder): IDecorationMeta {
		...
		if (parentMeta) {
			const ownMeta: IDecorationMeta = {
				...
				inheritable: item.type === EnumTreeItemType.Folder ? new DecorationComposite(item, DecorationCompositeType.Inheritable, parentMeta.inheritable) : null,
			}
	...
	private targetDecoration = (decoration: Decoration, target: BirchItem | BirchFolder): void => {
	...
	private unTargetDecoration = (decoration: Decoration, target: BirchItem | BirchFolder): void => {
		const { applicable, inheritable } = this.getDecorationData(target)
	...
	private negateDecoration = (decoration: Decoration, target: BirchItem | BirchFolder): void => {
		const { applicable, inheritable } = this.getDecorationData(target)
	...
	private unNegateDecoration = (decoration: Decoration, target: BirchItem | BirchFolder): void => {
		const { applicable, inheritable } = this.getDecorationData(target)
	...
	private switchParent = (target: BirchItem | BirchFolder, prevParent: BirchFolder, newParent: BirchFolder): void => {
