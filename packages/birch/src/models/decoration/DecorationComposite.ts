...
import { DecorationTargetMatchMode } from './Decoration'
...
export class DecorationComposite {
	...
	constructor(target: BirchItemOrFolder, type: DecorationCompositeType, parent: DecorationComposite) {
		this.target = target
		this.type = type
	...
	public add(decoration: Decoration): void {
		...
		const applicableToSelf = applicationMode && (applicationMode === DecorationTargetMatchMode.Self || applicationMode === DecorationTargetMatchMode.SelfAndChildren)
		const applicableToChildren = applicationMode && (applicationMode === DecorationTargetMatchMode.Children || applicationMode === DecorationTargetMatchMode.SelfAndChildren)
	...
	public negate(decoration: Decoration): void {
		...
		const negatedOnSelf = negationMode && (negationMode === DecorationTargetMatchMode.Self || negationMode === DecorationTargetMatchMode.SelfAndChildren)
		const negatedOnChildren = negationMode && (negationMode === DecorationTargetMatchMode.Children || negationMode === DecorationTargetMatchMode.SelfAndChildren)
	...
	private parentOwn(newParent?: DecorationComposite, reason?: ChangeReason, decoration?: Decoration) {
		this.selfOwned = false
		this.targetedDecorations = void 0 as any
		this.negatedDecorations = void 0 as any
	...
	private processCompositeAlteration(reason: ChangeReason, decoration: Decoration): boolean {
		...
		if (reason === ChangeReason.TargetDecoration) {
			...
			if (negatedOnSelf && this.type === DecorationCompositeType.Applicable) { return false }

			if (negatedOnChildren && this.type === DecorationCompositeType.Inheritable) { return false }
