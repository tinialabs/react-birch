...
export interface ITreeViewHandle<T> {
     ...
     */
    onDidExpandElement(handler: (event: ITreeViewExpansionEvent) => void): Disposer

    /**
   * Event that is fired when an element is collapsed
   */
    onDidCollapseElement(handler: (event: ITreeViewExpansionEvent) => void): Disposer

    /**
     * Event that is fired when the [selection](#TreeViewHandle.selection) has changed
     */
    onDidChangeSelection(handler: (event: ITreeViewSelectionChangeEvent) => void): Disposer

    /**
     * Event that is fired when [visibility](#TreeViewHandle.visible) has changed
     */
    onDidChangeVisibility(handler: (event: ITreeViewVisibilityChangeEvent) => void): Disposer
     ...
     */
    reveal(element: ITreeItem, options?: { select?: boolean, focus?: boolean, expand?: boolean | number }): Promise<void>;
