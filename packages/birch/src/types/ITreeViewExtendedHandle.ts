...
export interface ITreeViewExtendedHandle {
	 ...
	 */
	getItemHandle(path: string, expandTree?: boolean): Promise<BirchItem | BirchFolder>


	// EVENTS //
	onDidChangeModel(callback: (prevModel: TreeViewModel, newModel: TreeViewModel) => void): IDisposable
	onceDidChangeModel(callback: (prevModel: TreeViewModel, newModel: TreeViewModel) => void): IDisposable
