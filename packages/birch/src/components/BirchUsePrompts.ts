...
export const usePrompts = (
  ...
  const updatePromptHandle = useCallback(
    (handle: IPromptHandleNewItem | IPromptHandleRename) => {
      ...
      if (
        promptHandle.current instanceof PromptHandle &&
        !promptHandle.current.destroyed
      ) {
        promptHandle.current.destroy()
      }
  ...
  const adjustedRowCount = useMemo(() => {
    ...
    return newItemPromptInsertionIndex.current > -1 &&
      ...
      promptHandle.current.constructor === PromptHandleNewItem &&
      !promptHandle.current.destroyed
  ...
  const getItemAtIndex = useCallback(
    (index: number): IBirchTreeItemProps => {
      ...
      if (!cached) {
        ...
        if (
          ...
          if (index === promptInsertionIdx) {
            cached = {
              itemType:
                (promptHandle.current as PromptHandleNewItem).type ===
                EnumTreeItemType.Item
                  ? EnumBirchItemType.NewItemPrompt
                  : EnumBirchItemType.NewFolderPrompt,
          ...
          if (
            ...
            promptHandle.current &&
            promptHandle.current.constructor === PromptHandleRename &&
            (promptHandle.current as PromptHandleRename).originalLabel ===
              item.label &&
            !promptHandle.current.destroyed
          ) {
            cached = {
