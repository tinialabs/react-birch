...
export const useDragDropContainer = (birchContextRef: React.MutableRefObject<IBirchContext>) => {
    ...
    useEffect(() => {
        ...
        dragAndDropService.current = new DragAndDropService(birchContextRef.current.model), [viewId]


        dragAndDropService.current.onDragAndDrop(async (item: BirchItemOrFolder, newParent: BirchFolder) => {
            ...
            try {
                ...
                const allowed = await treeDataProvider.moveItem(item, newParent, newPath)
                if (!allowed) { return; }
                model.root.dispatch({
